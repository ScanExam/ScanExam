...
class FormattedTextField extends TextField {
	...
	new() {
		...
		disabledProperty.addListener(obs, oldVal, newVal|newVal ? wrongFormatted = false : updateState)
	}
	...
	def setDefaultValue(String value) {
		...
		if (pattern.matcher(value).matches) {
			...
			this.focusedProperty.addListener [ obs, oldVal, newVal |
				!newVal && (text === null || text == "") ? this.text = defaultValue.value
			]
			if (text === null || text == "")
				text = defaultValue.value
		} else
	...
	def addFormatValidator(FormatValidator validator) {
		...
		focusedProperty.addListener [ obs, oldVal, newVal | !newVal ? evaluateValidator(validator) ]
		textProperty.addListener [ obs, oldVal, newVal | oldVal != newVal && !focused ? evaluateValidator(validator) ]
		onKeyPressed = [event | event.code === KeyCode.ENTER ? evaluateValidator(validator)]
	}
	 ...
	 */
	private def evaluateValidator(FormatValidator validator) {
	 ...
	 */
	def void shakeIfEmpty() {
		if (!disabled && (text === null || text == ""))
			shake()
	}
