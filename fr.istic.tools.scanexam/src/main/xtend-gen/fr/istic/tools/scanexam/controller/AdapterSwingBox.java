...
import fr.istic.tools.scanexam.presenter.SelectionPresenter;
...
public class SelectionPresenterSwing extends SelectionPresenter {
  /**
   ...
   */
  public SelectionPresenterSwing(final int windowWidth, final int windowHeight, final int scale, final int originX, final int originY, final BoxList selectionBoxes) {
    ...
    this.mouseHandler = new MouseAdapter() {
      ...
      public void mousePressed(final MouseEvent e) {
        ...
        if (_tripleEquals) {
          SelectionPresenterSwing.this.crossCliked = false;
          SelectionPresenterSwing.this.lastClickPoint = Optional.<Point>of(e.getPoint());
          Optional<Box> pointedBox = SelectionPresenterSwing.this.checkPoint(e.getPoint());
          boolean _isPresent = pointedBox.isPresent();
          boolean _not = (!_isPresent);
          if (_not) {
            SelectionStateMachine.setState(SelectionStateMachine.CREATE);
            SelectionPresenterSwing.this.createBox(e);
            SelectionStateMachine.setState(SelectionStateMachine.RESIZE);
            SelectionPresenterSwing.this.resizeBox(e, SelectionPresenterSwing.this.lastBoxSelected);
          } else {
            SelectionPresenterSwing.this.lastBoxSelected = pointedBox.get();
            if ((!SelectionPresenterSwing.this.crossCliked)) {
              SelectionStateMachine.setState(SelectionStateMachine.MOVE);
              SelectionPresenterSwing.this.moveBox(e, SelectionPresenterSwing.this.lastBoxSelected);
            } else {
              SelectionStateMachine.setState(SelectionStateMachine.DELETE);
              SelectionPresenterSwing.this.deleteBox(SelectionPresenterSwing.this.lastBoxSelected);
            }
          }
        }
      }
      ...
      public void mouseReleased(final MouseEvent e) {
        ...
        SelectionPresenterSwing.this.lastClickPoint = Optional.<Point>empty();
      }
