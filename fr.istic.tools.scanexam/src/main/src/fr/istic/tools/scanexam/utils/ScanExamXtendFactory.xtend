...
class ScanExamXtendFactory {
	...
	def static gradingData(Exam exam) {
		...
		val pngfiles = dir.listFiles(new FilenameFilter() {
			override boolean accept(File dir, String name) {
				return name.toLowerCase().endsWith(".png");
			}
		})
		if (pngfiles===null || pngfiles.empty) {
			throw new UnsupportedOperationException('''No png files found in folder «exam.folderPath» ''')
	
		}
		e.images+= pngfiles.sortInplace([File a, File b | a.name.compareTo(b.name)]);
		
		if (e.images.size%exam.numberOfPages!=0) {
			throw new UnsupportedOperationException('''Inconsistent number of pages «e.images.size» images found, should b multiple by «exam.numberOfPages»  ''')
		}
